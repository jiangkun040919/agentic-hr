...
> 截取以下页面并保存到 `docs/screenshots/` 目录，即可在下方展示。  
> 建议截图分辨率：1920×1080，PNG 格式。
...
岗位卡片式展示，支持关键词搜索、部门筛选和分页。每个卡片展示岗位名称、部门、薪资范围、技能标签。顶部导航支持求职者快速切换浏览/投递/面试。

#### 岗位详情与投递
![岗位详情](docs/screenshots/job-detail.png)

岗位详情页展示完整 JD、技能要求、薪资福利。底部「立即投递」按钮触发单页投递表单，支持 PDF/Word 简历上传（base64 编码），姓名/手机/邮箱/学历必填校验。

#### AI 智能面试
![AI面试](docs/screenshots/ai-interview.png)

全屏面试界面，左侧显示岗位信息和面试须知，右侧为对话区。AI 自动提问并根据回答评分，支持**文字输入**和**语音输入**两种模式。顶部实时显示轮次进度、面试时长、已答题数。摄像头区域集成了 TensorFlow.js **行为分析**（面部检测、姿态识别、注意力评估），面试官语音播报（TTS）。

#### 面试报告
![面试报告](docs/screenshots/ai-interview-report.png)

面试结束后即时生成的雷达图评分报告，覆盖**专业能力、沟通表达、问题解决、文化适配**四个维度。展示综合评分、各轮次对话记录和 AI 评语。

#### 我的投递
![我的投递](docs/screenshots/my-deliveries.png)

双 Tab 页面——「我的投递」展示所有投递记录及状态流转（待查看→已查看→面试中→已入职→已淘汰），顶部 AI 面试邀请卡片突出显示待参加面试；「AI 面试记录」展示已完成/进行中的 AI 面试历史，含评分环、时长、状态标签。

#### 推荐岗位
![推荐岗位](docs/screenshots/recommended-jobs.png)

基于候选人技能和投递历史的 AI 推荐。横向卡片滑动展示，每张卡片含**匹配度百分比环**、AI 推荐理由、技能匹配标签。
...
HR 工作台首页，展示关键指标卡片（总岗位数、投递量、面试通过率、本月入职），招聘漏斗图（投递→筛选→面试→录用），近期投递动态时间线。

#### 岗位管理
![岗位管理](docs/screenshots/job-management.png)

岗位 CRUD 表格，支持批量操作（发布/下架/删除）、多条件筛选、Excel 导入导出。每条岗位显示招聘进度（投递数/面试数/入职数）。

#### 简历管理
![简历管理](docs/screenshots/resume-management.png)

投递简历列表，支持按岗位/状态/时间筛选。每条记录展示候选人信息、简历文件预览、AI 解析的技能标签。可批量下载简历压缩包。

#### 智能筛选
![智能筛选](docs/screenshots/smart-screening.png)

AI 自动评分排序页面，展示候选人与岗位的匹配度百分比、技能覆盖度、经验匹配度。支持一键邀请面试。

#### AI 面试记录管理
![AI面试管理](docs/screenshots/ai-interview-management.png)

HR 查看所有候选人的 AI 面试记录，包含候选人姓名、岗位、评分、时长、状态。点击可展开完整对话回放和四维评分详情。

#### 人才对比
![人才对比](docs/screenshots/candidate-comparison.png)

多候选人横向对比视图，技能雷达图叠加展示，匹配度排序，直观比较候选人优劣势。

#### 知识图谱
![知识图谱](docs/screenshots/knowledge-graph.png)

Neo4j 驱动的岗位-技能-候选人关联图谱，G6 力导向图渲染。Obsidian 深色画布风格，黏土 3D 节点带光晕效果，技能粒子流动动画。支持拖拽、缩放、点击节点查看详情。被 4 个管理端页面共用（知识图谱、人才对比、简历详情、智能筛选）。

#### 招聘策略分析
![招聘策略](docs/screenshots/recruitment-strategy.png)

漏斗转化率分析、渠道效果对比、时间趋势图。Pipeline 阶段统计（投递/筛选/面试/录用各阶段人数和转化率）。

#### 合规审计
![合规审计](docs/screenshots/compliance.png)

招聘合规性检查页面，公平性分析指标（性别比例、年龄分布、学历构成），数据隐私合规报告。
