      ...
      <el-table :data="jobs" stripe @selection-change="onSelectionChange" class="jm-table">
        ...
        <el-table-column label="薪资" width="120">
          <template #default="{ row }">
            <span v-if="row.salaryMin&&row.salaryMax" class="tbl-salary">{{ row.salaryMin }}-{{ row.salaryMax }}K</span>
    ...
    <div v-else class="jm-cards" v-loading="loading">
      <el-card v-for="job in jobs" :key="job.jobId" class="jm-card-item" shadow="hover"
        ...
        <div class="card-meta">
          ...
          <span v-if="job.salaryMin" class="meta-salary">{{ job.salaryMin }}-{{ job.salaryMax }}K</span>
        </div>
...
import { Plus, Search, Location, Grid, List } from '@element-plus/icons-vue'
...
const deptColor = (d: string) => ({'技术部':'#409EFF','AI部':'#9B59B6','数据部':'#67C23A','产品部':'#E67C1A','运营部':'#F56C6C','市场部':'#36CFC9','财务部':'#2C3E50','人力资源部':'#FF85C0'}[d] || '#409EFF')
